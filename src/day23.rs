...
#[derive(Debug, Clone)]
pub struct Burrow {
    rooms: HashMap<Amp, [Point2<i32>; 2]>,
...
impl Burrow {
    ...
    fn can_move(&self, amp: Amp, from: Point2<i32>, to: Point2<i32>) -> bool {
        ...
        // Add a test here to find out if it's already in it's room
        // and doesn't need to move.

        if is_room {
            let your_rooms = self.rooms.get(&amp).unwrap();
            if your_rooms[1] == from
...
impl FromStr for Burrow {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let mut rooms = [
            (Amp::Amber, [rooms[0], rooms[1]]),
            (Amp::Bronze, [rooms[2], rooms[3]]),
            (Amp::Copper, [rooms[4], rooms[5]]),
            (Amp::Desert, [rooms[6], rooms[7]]),
        ]
        .into_iter()
        .collect();
...
impl Day for Solution {
    ...
    fn p1(v: &Self::Input) -> Self::Sol1 {
        ...
        dfs(v.clone(), &node_indices, &ungraph, &dists, 0, None).unwrap()
    }
